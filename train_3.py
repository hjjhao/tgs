...
def train(net=None):
    ...
    logger_init(log_filename='res_unet_train_log', log_level=logging.INFO, log_dir=config.SAVED_LOGS_PATH)
    ...
    train_dataset = dataset.SaltDataset(train_list, depth_csv, 'train', transform)
    valid_dataset = dataset.SaltDataset(valid_list, depth_csv, 'valid', transform)
    ...
    for epoch in range(config.EPOCHS):
        ...
        net.train()
        for imgs, masks, depths in tqdm.tqdm(train_loader):
            imgs, masks,depths = imgs.to(config.DEVICE), masks.to(config.DEVICE), depths.to(config.DEVICE)
        ...
        with torch.no_grad():
            for imgs, masks, depths in valid_loader:
                imgs, masks,depths = imgs.to(config.DEVICE), masks.to(config.DEVICE), depths.to(config.DEVICE)
                outputs = net([imgs, depths])
                iou_pred = (torch.sigmoid(outputs) > config.THRESHOLD)
                ma = (masks > 0.5)
        ...
        logging.info("[INFO]---valid: loss={:.6f}   iou={:.6f}\n".format(epoch_valid_loss, epoch_valid_iou))
    ...
    plt.savefig(os.path.join(config.SAVED_PLOT_PATH, 'loss_plot_3.png'))
...
#plot iou list
    ...
    plt.savefig(os.path.join(config.SAVED_PLOT_PATH, 'iou_plot_3.png'))
